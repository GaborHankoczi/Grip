...
{
#ifdef NDEF_DEBUG
/*Serial.print(F("Decoding "));Serial.print(numBytes);Serial.println(F(" uint8_ts"));
...
// DumpHex(data, numBytes, 16);
    ...
    while (index <= numBytes)
    {
        ...
        {
            payloadLength = ((0xFF & data[++index]) << 24);
            payloadLength |= ((0xFF & data[++index]) << 16);
            payloadLength |= ((0xFF & data[++index]) << 8);
            payloadLength |= (0xFF & data[++index]);
        }
...
void NdefMessage::addMimeMediaRecord(string mimeType, uint8_t *payload, int payloadLength)
{
    ...
    const char *type_str = mimeType.c_str();
    uint16_t type_len = mimeType.length();
    memcpy(type, type_str, type_len + 1);
...
void NdefMessage::addTextRecord(string text, string encoding)
{
    ...
    const char *payload_str = payloadstring.c_str();
    uint16_t payload_len = payloadstring.length();
    memcpy(payload, payload_str, payload_len + 1);
...
void NdefMessage::print()
{
    ...
    Serial.print(getEncodedSize());Serial.println(F(" uint8_ts"));
